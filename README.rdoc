...
2. Generate the basic trucker files
    ...
    * Adds legacy adapter to database.yml
    * Adds legacy base class
    * Adds legacy sub classes for all existing models
    * Adds app/models/legacy to load path in Rails Initializer config block
    * Generates sample migration task (using pluralized model names)
